...
//    std::cout << ft.front() << std::endl;
   ...
   std::cout << ")." << std::endl;
   const int k = 8;
   ft::allocator<int>::const_pointer v1Ptr;
      ft::allocator<int>::const_pointer v2Ptr;
 //  v1Ptr = v1Alloc.address( *find(v1.begin( ), v1.end( ), k) );
    v1Ptr = v1Alloc.address( k );
   std::cout << "The integer addressed by v1Ptr has a value of: "
        << "*v1Ptr = " << *v1Ptr << "." << std::endl;
 //   ft::allocator<int> Alloc;
   ft::allocator<int> cAlloc ;
   cAlloc = v1Alloc;
    v2Ptr = cAlloc.address( k );
   std::cout << "*v2Ptr = " << *v2Ptr << "." << std::endl;  
    ft::allocator<int> v2Alloc;
   ft::allocator<int>::pointer v2aPtr;
       v2aPtr = v2Alloc.allocate ( 10 );
   int i2;
   for ( i2 = 0 ; i2 < 10 ; i2++ )
   {
      v2aPtr[ i2 ] = i2;
   ...
   std::cout << std::endl;
   v1Alloc.deallocate( v2aPtr, 10 );

    ft::allocator<int>::pointer v1PtrA;
   int kA = 6, kB = 7;
   v1PtrA = v1Alloc.address( *find( v1.begin( ), v1.end( ), kA ));
   v1Alloc.destroy ( v1PtrA );
   v1Alloc.construct ( v1PtrA , kB );
   std::cout << "The modified vector v1 is:\n ( " ;
   for ( v1Iter = v1.begin( ) ; v1Iter != v1.end( ) ; v1Iter++ )
      std::cout << *v1Iter << " ";
   std::cout << ")." << std::endl; 
    return (0);
